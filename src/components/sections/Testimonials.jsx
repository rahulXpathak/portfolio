  ...
  return (
    <section id="testimonials" className="py-20 md:py-32 bg-black border-t border-white/10 relative perspective-1000 overflow-hidden">
      ...
      <div className="flex overflow-hidden w-full [mask-image:linear-gradient(to_right,transparent,black_5%,black_95%,transparent)]">
        ...
        <div className="flex flex-nowrap gap-4 md:gap-8 animate-loop-scroll hover:pause py-10 md:py-20 px-4">
          ...
          {[...certifications, ...certifications].map((cert, i) => (
            <div
              ...
              className="
                  group
                  min-w-[280px] md:min-w-[450px] 
                  h-auto min-h-[260px] md:h-[300px]
                  bg-neutral-900/40 
                  p-6 md:p-10 
            ...
            >
              ...
              {/* Decorative Icon */}
              <div className="text-blue-600 opacity-50 mb-3 md:mb-4 group-hover:text-blue-400 group-hover:scale-110 transition-all duration-300 origin-left">
                <Award size={32} className="md:w-10 md:h-10" strokeWidth={1} />
              </div>

              <p className="text-sm md:text-lg text-white font-light leading-relaxed mb-3 md:mb-4 relative z-10 group-hover:text-blue-50 transition-colors">
                {cert.desc}
              </p>

              <div className="flex items-center gap-3 md:gap-4 mt-auto border-t border-white/10 pt-4 md:pt-6 relative z-10">
                <div className="
                    w-10 h-10 md:w-12 md:h-12 
                    bg-gradient-to-tr from-blue-600 to-purple-600 
                    rounded-lg 
                    flex items-center justify-center 
                    font-bold text-white text-xs md:text-sm 
                    shadow-lg 
                ...
                <div className="min-w-0">
                  <h4 className="text-white font-bold text-xs md:text-sm tracking-wide group-hover:text-blue-200 transition-colors truncate">{cert.title}</h4>
                  <p className="text-gray-500 text-[10px] md:text-xs uppercase tracking-wider group-hover:text-gray-400 transition-colors">{cert.issuer}</p>
                </div>
              </div>
            </div>
          ))}
